...
do
    ...
    # do
    #     python -m scripts.make_ngrams -w $cdir -a $vsz -x $nmode -n 2 -d ${bcdir}/train.tsv -v src ${vcb_dir}/word.max.${sl}.model tgt ${vcb_dir}/word.max.${tl}.model -b src ${vcb_dir}/${vpr}${sl}.model tgt ${vcb_dir}/${vpr}${tl}.model -m src ${vcb_dir}/match.${vpr}${sl}.model tgt ${vcb_dir}/match.${vpr}${tl}.model
    #     for ns in ${tokens[@]}
    #     do 
    #         for mode in ${modes[@]}
    #         do
