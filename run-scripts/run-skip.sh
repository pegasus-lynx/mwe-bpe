...
do
    ...
    do
        ...
        if [ $shared -eq 1 ]; then
            python -m make_conf -n prep.yml -w $exp_dir -c $base_prep_file -r $repo_root --kwargs pieces=$pieces max_types=$sz max_skipgrams=$skip_sz
        else
            python -m make_conf -n prep.yml -w $exp_dir -c $base_prep_file -r $repo_root --kwargs max_src_types=$sz max_tgt_types=$sz max_skipgrams=$skip_sz src_pieces=$src_pieces tgt_pieces=$tgt_pieces
        fi
